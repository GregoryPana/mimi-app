  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      ...
      ref
          ...
          .updateLastGalleryFolder(AppConfig.galleryFolders[_selectedFolder] ?? _selectedFolder);

      final shouldShow =
          await ref.read(progressControllerProvider.notifier).showGalleryIntroIfNeeded();
            ...
            child: Row(
              children: [
                ...
                _TabButton(
                  ...
                  onTap: () => setState(() => _currentTab = GalleryTab.shared),
                ),
              ],
            ),
          ),
          ...
          child: AnimatedSwitcher(
            ...
            child: _currentTab == GalleryTab.ours
                ? contentAsync.when(
                    data: (content) {
                      final filtered = content.gallery
                          ...
                          .toList();
                      final viewedIds =
                          ref.watch(progressControllerProvider).value?.galleryViewedIds ??
                              <String>{};
                      return Column(
                        children: [
                          _FolderSwitcher(
                ...
                : _currentTab == GalleryTab.yours
                    ...
                    : ref.watch(sharedCollectionsProvider).when(
                        data: (cols) => _SharedCollectionsView(
                          collections: cols,
                        ),
        ...
        return GestureDetector(
          ...
          child: Stack(
            children: [
              ...
              .animate()
              .fadeIn(duration: 350.ms, delay: (index * 25).ms)
        ...
        PastelCard(
          child: ListTile(
            leading: Container(
              ...
              child: const Icon(LucideIcons.plusCircle, color: AppColors.pastelPink, size: 20),
            ),
                  ...
                  child: Column(
                    ...
                    children: [
                      Icon(LucideIcons.heart,
                          size: 48, color: AppColors.pastelPink.withValues(alpha: 0.3)),
                      const SizedBox(height: 12),
                      Text('Create your first collection, Baby ✨',
                          ...
                          leading: col.images.isNotEmpty
                              ...
                              : Container(
                                  ...
                                  child: const Icon(LucideIcons.image,
                                      color: AppColors.pastelLavender, size: 20),
                                ),
                          title: Text(col.name,
                              style: const TextStyle(fontWeight: FontWeight.w600)),
                          subtitle: Text('${col.images.length} memories',
                              style: Theme.of(context).textTheme.bodySmall),
                          trailing: const Icon(Icons.chevron_right,
                              color: AppColors.textMuted, size: 20),
                          onTap: () => Navigator.of(context).push(MaterialPageRoute(
                              builder: (_) => UserCollectionScreen(collection: col))),
                        ),
                      ).animate().fadeIn(duration: 400.ms, delay: (index * 40).ms).slideX(begin: 0.04),
                    );
                  },
                ),
        ),
...
class UserCollectionScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final current =
        collections.firstWhere((c) => c.id == collection.id, orElse: () => collection);
                                ...
                                Positioned(
                                  ...
                                  child: Consumer(
                                    builder: (context, ref, child) {
                                      ...
                                      final isFav =
                                          progress?.favoriteIds.contains(img.id) ?? false;
                                      return GestureDetector(
                                        onTap: () {
                                          HapticFeedback.lightImpact();
                                          ref
      ...
      builder: (context) => AlertDialog(
        ...
        actions: [
          ...
          TextButton(
            onPressed: () {
              if (controller.text.isNotEmpty) {
                ref.read(userGalleryControllerProvider.notifier).addImageToCollection(
                    colId,
                    pickedFile.path,
                    controller.text,
                  );
...
class _GalleryViewerScreenState extends ConsumerState<GalleryViewerScreen> {
  ...
  String _heroTagAt(int index) {
    if (widget.oursItems != null) return 'gallery-${widget.oursItems![index].id}';
    if (widget.sanityItems != null) return 'sanity-${widget.sanityItems![index].id}';
  ...
  Widget _imageAt(int index) {
    if (widget.oursItems != null) {
      return Image.asset(widget.oursItems![index].imageAsset, fit: BoxFit.contain);
    }
    if (widget.sanityItems != null) {
      return CachedNetworkImage(
          imageUrl: widget.sanityItems![index].imageUrl, fit: BoxFit.contain);
    }
    return Image.file(File(widget.yoursItems![index].filePath), fit: BoxFit.contain);
  }
...
class _BottomInfoBar extends ConsumerWidget {
  const _BottomInfoBar({super.key, required this.caption, required this.itemId});
          ...
          child: Row(
            children: [
              ...
              GestureDetector(
                onTap: () {
                  HapticFeedback.lightImpact();
                  ref.read(progressControllerProvider.notifier).toggleFavorite(itemId);
                },
                child: AnimatedContainer(
                  duration: const Duration(milliseconds: 200),
          ...
          return Expanded(
            child: Padding(
              ...
              child: ChoiceChip(
                ...
                selectedColor: AppColors.pastelPeach.withValues(alpha: 0.8),
                ...
                child: Row(
                  ...
                  children: const [
                    Icon(LucideIcons.plusCircle,
                        size: 14, color: AppColors.pastelPink),
                    SizedBox(width: 6),
                    Text('New',
                        style: TextStyle(
                            fontSize: 13,
                  ...
                  child: Column(
                    ...
                    children: [
                      Icon(LucideIcons.imageOff,
                          ...
                          color: AppColors.pastelPink.withValues(alpha: 0.3)),
                      const SizedBox(height: 12),
                      const Text('No shared collections yet',
                      ...
                      return Padding(
                        ...
                        child: PastelCard(
                          child: ListTile(
                            ...
                            onTap: () => Navigator.of(context).push(
                              MaterialPageRoute(
                                builder: (_) => SharedCollectionScreen(
                                  collection: col,
                                ),
                              ),
                            ).then((_) {
          ...
          TextButton(
            onPressed: () async {
              ...
              await repo.createSharedCollection(
                  name: ctrl.text.trim(), createdBy: author);
              ref.invalidate(sharedCollectionsProvider);
            },
            child: const Text('Create'),
          ),
...
class _SharedCollectionScreenState
  ...
  Widget build(BuildContext context) {
    ...
    final col = collsAsync.valueOrNull
            ?.firstWhere((c) => c.id == widget.collection.id,
    ...
    return Scaffold(
            ...
            const Padding(
              ...
              child: Center(
                child: SizedBox(
                    ...
                    height: 20,
                    child: CircularProgressIndicator(
                        strokeWidth: 2, color: AppColors.pastelPink)),
              ),
            ),
        ],
      ),
      body: AnimatedGradientBackground(
        child: Padding(
          padding: EdgeInsets.fromLTRB(
              20, MediaQuery.of(context).padding.top + kToolbarHeight + 12, 20, 12),
          child: Column(
            children: [
              PastelCard(
                child: ListTile(
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: TextField(
          ...
          autofocus: true,
          decoration:
              const InputDecoration(hintText: 'Describe this moment…'),
        ),
        actions: [
          TextButton(
  ...
  Future<void> _confirmDelete(
      String collectionId, String imageKey, String caption) async {
